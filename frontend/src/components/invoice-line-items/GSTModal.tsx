...
export default function GSTModal({ open, settings, onApply, onClose }: { open: boolean; settings: GSTSettings; onApply: (settings: GSTSettings) => void; onClose: () => void }) {
  ...
  if (!open) return null;
  const isGstIndia = draft.enabled && draft.taxType === 'GST (India)';
  const isOtherTax = draft.enabled && !isGstIndia;
  ...
  return (
    <div className="invoice-modal fixed inset-0 z-[90] flex items-center justify-center bg-slate-950/50 p-3" role="dialog" aria-modal="true" aria-labelledby="gst-modal-title">
      <div className="flex max-h-[94vh] w-full max-w-md flex-col overflow-hidden rounded-lg border border-slate-200 bg-white shadow-2xl">
        <header className="flex shrink-0 items-center justify-between border-b border-slate-200 px-5 py-4">
          <h2 id="gst-modal-title" className="text-[18px] font-semibold leading-6 text-slate-900">Configure Tax</h2>
        ...
        <div className="flex-1 space-y-5 overflow-y-auto px-5 py-5">
                ...
                <div className="absolute left-0 right-0 top-full z-20 mt-1 overflow-hidden rounded-md border border-slate-200 bg-white shadow-xl">
                  <div className="max-h-52 overflow-y-auto py-1" role="listbox">
                    {Array.from(new Set([...DEFAULT_TAX_TYPES, draft.taxType])).filter(Boolean).map((taxType) => {
                      ...
                      return <button key={taxType} type="button" role="option" aria-selected={selected} onClick={() => { setDraft((current) => ({ ...current, enabled: taxType !== 'NONE', taxType: taxType === 'NONE' ? current.taxType : taxType, type: taxType !== 'NONE' && taxType !== 'GST (India)' ? 'IGST' : current.type })); setTaxMenuOpen(false); }} className={`flex w-full items-center justify-between px-4 py-2.5 text-left text-xs ${selected ? 'bg-[#EAF4FF] font-bold text-[#2E6EAB]' : 'text-slate-700 hover:bg-slate-50'}`}><span>{taxType}</span>{selected ? <Check className="h-3.5 w-3.5" /> : null}</button>;
                    })}
                  </div>
                  <div className="border-t border-slate-200 p-2">
                    {creatingTax ? <div className="flex gap-2"><input autoFocus value={newTaxName} onChange={(event) => setNewTaxName(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && newTaxName.trim()) { event.preventDefault(); setDraft((current) => ({ ...current, enabled: true, taxType: newTaxName.trim(), type: 'IGST' })); setCreatingTax(false); setTaxMenuOpen(false); setNewTaxName(''); } }} placeholder="Tax name" className="min-w-0 flex-1 rounded-md border border-slate-300 px-2.5 py-2 text-xs outline-none focus:border-[#2E6EAB]" /><button type="button" disabled={!newTaxName.trim()} onClick={() => { setDraft((current) => ({ ...current, enabled: true, taxType: newTaxName.trim(), type: 'IGST' })); setCreatingTax(false); setTaxMenuOpen(false); setNewTaxName(''); }} className="rounded-md bg-[#2E6EAB] px-3 text-xs font-bold text-white disabled:opacity-40">Add</button></div> : <button type="button" onClick={() => setCreatingTax(true)} className="w-full rounded-md py-2 text-xs font-bold text-[#2E6EAB] hover:bg-[#EAF4FF]">+ Create New Tax</button>}
                  </div>
                </div>
              ) : null}
            </div>
          ...
          {isGstIndia ? <section className="grid grid-cols-[20px_1fr] gap-2">
          ...
          {isOtherTax ? <section className="grid grid-cols-[20px_1fr] gap-2">
            <span className="pt-0.5 text-xs font-black text-slate-800">2.</span>
