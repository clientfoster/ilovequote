...
function Cell({ label, children, className = '' }: { label: string; children: ReactNode; className?: string; key?: Key }) {
  return <div className={`mb-3 min-w-0 self-center last:mb-0 md:mb-0 ${className}`}><span className="mb-1 block text-[14px] font-medium leading-5 text-slate-500 md:hidden">{label}</span>{children}</div>;
}
